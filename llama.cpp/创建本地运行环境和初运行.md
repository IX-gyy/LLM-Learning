...
```
没有魔法可能下载困难，就算使用`export HF_ENDPOINT=https://hf-mirror.com`设置镜像站再下载也一样  
因为我们使用的是wsl，**可以直接在本机上下载相关文件再手动移动到对应的文件夹下**（即`我的电脑/Linux/home/用户名/llama.cpp/models`中）  
[点击下载链接](https://hf-mirror.com/TheBloke/TinyLlama-1.1B-Chat-v1.0-GGUF/blob/main/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf)  
...
打开的将会是一个交互式的命令行，可以进行若干次的对话，~~不过模型太小，对话内容可以有点呆~~  
相关命令：
```
...
甚至还有Web页面（在`http://localhost:8080`打开）：
```
./build/bin/llama-server \
  ...
  --port 8080
```
不得不承认，1.1的聊天功能有点像跨服聊天hhh :(  
不过我们只是想要初步使用`llama.cpp`！现在的结果就是已经成功了！我们是————🏆！
